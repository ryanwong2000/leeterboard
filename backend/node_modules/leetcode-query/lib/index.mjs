"use strict";import O from"events";import E from"node-fetch";var x=class{constructor(){this._table={}}get(i){let e=this._table[i];if(e){if(e.expires>Date.now())return e.value;this.remove(i)}return null}set(i,e,t=6e4){this._table[i]={key:i,value:e,expires:t>0?Date.now()+t:0}}remove(i){delete this._table[i]}clear(){this._table={}}load(i){this._table=JSON.parse(i)}},h=new x,j={default:h};var g="https://leetcode.com",f="https://leetcode.cn",d="Mozilla/5.0 LeetCode API";import D from"node-fetch";function p(u){return u.split(";").map(i=>i.trim().split("=")).reduce((i,e)=>(i[e[0]]=e[1],i),{})}async function A(){let u=await D(g,{headers:{"user-agent":d}}).then(e=>e.headers.get("set-cookie"));return p(u).csrftoken}var y=class{constructor(i){i&&(this.session=i.session,this.csrf=i.csrf)}async init(i){return this.csrf=await A(),i&&(this.session=i),this}};var R=`
